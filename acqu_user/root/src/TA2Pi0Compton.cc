...
// Create arrays to hold Particles
	...
	fTaggerPi0TimeCut	= new Double_t[352*fMaxNParticle];
...
//Here
	TA2DataManager::LoadVariable("2PhotonPhiDiff", 		&f2PhotonPhiDiff,		EDSingleX);
	TA2DataManager::LoadVariable("CBESumCut",				&fCBESumCut,			EDSingleX);
	...
	TA2DataManager::LoadVariable("TaggerPi0TimeCut",	fTaggerPi0TimeCut,			EDMultiX);
	...
	for (i = 0; i < fNPhoton; i++)  {
		...
		for (j = 0; j < fNTagg; j++) {
			...
			if ( (fTaggerPhotonTime[fNTaggNPhot] >= fPhotTimePL && fTaggerPhotonTime[fNTaggNPhot] <= fPhotTimePR) ||
				...
				fMissingEnergyPrompt[fNPrompt]	= p4missing.M()-fP4target[0].M();
			...
			if ( (fTaggerPhotonTime[fNTaggNPhot] >= fPhotTimeRL1 && fTaggerPhotonTime[fNTaggNPhot] <= fPhotTimeRR1) ||
				...
				fMissingEnergyRandom[fNRandom]	= p4missing.M()-fP4target[0].M();
...
// Here
	...
	if ( fNPhoton == 2)
	{
		...
		phi_diff = fabs( fPhotonPhi[0] - fPhotonPhi[1]);

		f2PhotonPhiDiff = phi_diff;
//		std::cout << phi_diff << std::endl;
		
		if (( phi_diff >= 160) && ( phi_diff <= 200))
		{
			fCBESumCut = (Float_t)(fNaI->GetTotalEnergy());
		}
	...
	for (i = 0; i < fNPi0; i++)  {
		...
		for (j = 0; j < fNTagg; j++) {
			...
			if ( (fTaggerPi0Time[fNTaggNPi0] >= fPi0TimePL && fTaggerPi0Time[fNTaggNPi0] <= fPi0TimePR) ||
				...
				if ( ( eBeamPhot >= 280) && ( eBeamPhot <= 340))
				{
					fMissingEnergyPromptPi0Cut[fNPromptPi0]	= p4missing.M()-fP4target[0].M();
				}
			...
			if ( (fTaggerPi0Time[fNTaggNPi0] >= fPi0TimeRL1 && fTaggerPi0Time[fNTaggNPi0] <= fPi0TimeRR1) ||
				...
				if ( ( eBeamPhot >= 280) && ( eBeamPhot <= 340))
				{
					fMissingEnergyRandomPi0Cut[fNRandomPi0]	= p4missing.M()-fP4target[0].M();
				}
...
/*	if ( fNaINCluster == 2)
	{
		Double_t phi1, phi2, phi_diff;

		phi1 = (Float_t)(fNaI->fCluster(0)->GetPhi());
		phi2 = (Float_t)(fNaI->fCluster(1)->GetPhi());

		phi_diff = fabs( phi1 - phi2);
...
// Apply BufferEnd to the end of all arrays
	...
	fTaggerPi0TimeCut[fNTaggNPi0]	= EBufferEnd;
	...
	fMissingEnergyPromptPi0Cut[fNPromptPi0]	= EBufferEnd;
	fMissingEnergyRandomPi0Cut[fNRandomPi0]	= EBufferEnd;
